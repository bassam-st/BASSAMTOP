...
class LearnRunIn(BaseModel):
    topics: Optional[List[str]] = None   # لو فاضي يستخدم TOPICS الافتراضية
...
@router.get("/news")
async def news():
    # <== لا نستخدم db/sqlite هنا لكي لا تظهر 500 لو ما فيه جدول
    return {"docs": get_latest_results(limit=20)}
...
async def learn_state():
    st = get_status()
    # واجهة الهوم تبحث عن 'state' أو 'status'
    st["state"] = "running" if st.get("running") else "idle"
    return st
